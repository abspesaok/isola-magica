...
/* ═══════════════════════════════════════════════════════════
   SILVANA E IL REGNO INCANTATO — v2
   Motore a isole · Progressi salvati · Gemme Perdute (ripasso)
   Isola 1: Regno delle Gemme (colori + numeri)
   Isola 2: Bosco delle Creature Magiche (animali)
   ═══════════════════════════════════════════════════════════ */
...
function useSpeak() {
  ...
  return useCallback((text, rate = 0.8) => {
    if (!window.speechSynthesis) return;
    window.speechSynthesis.cancel();
    window.speechSynthesis.resume();
    const u = new SpeechSynthesisUtterance(text);
    u.lang = "en-US"; u.rate = rate; u.pitch = 1.15;
    if (voiceRef.current) u.voice = voiceRef.current;
    window.speechSynthesis.speak(u);
  }, []);
}

/* ─── Persistent progress (window.storage) ─── */
const DEFAULT_PROGRESS = { gems: 0, stars: {}, weak: {} };

async function loadProgress() {
  try {
    const res = await window.storage.get(STORAGE_KEY);
    if (res?.value) return { ...DEFAULT_PROGRESS, ...JSON.parse(res.value) };
...
export default function App() {
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      try { await window.storage.set(STORAGE_KEY, JSON.stringify(progress)); }
  ...
  const unlockAudio = () => {
    try {
      window.speechSynthesis.cancel();
      window.speechSynthesis.resume();
      const u = new SpeechSynthesisUtterance("Welcome to the Magic Kingdom, Silvana!");
      u.lang = "en-US"; u.rate = 0.85; u.pitch = 1.15;
      window.speechSynthesis.speak(u); // synchronous, inside the tap: unlocks mobile audio
          ...
          <div className="flex flex-col gap-4 w-full items-center">
            {currentIsland.games.map((g) => (
              <button key={g.key} className="game-tile"
                onClick={() => { window.speechSynthesis?.resume?.(); setView({ screen: "game", islandId: currentIsland.id, gameKey: g.key }); }}>
